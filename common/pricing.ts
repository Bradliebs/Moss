...
// null so callers show nothing rather than fabricating a number.
//
// Lives in common/ (pure data + functions, no environment-specific imports) so
// both the React renderer (cost readouts) and the Electron main process (the
// daily-budget enforcer) share one rate table instead of duplicating it.
