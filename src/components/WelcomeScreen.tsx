  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-8 px-4 text-center animate-fade-in">
      <div className="flex flex-col items-center gap-3">
        ...
        <h1 className="text-3xl font-semibold tracking-tight text-neutral-50">Moss</h1>
        <p className="text-sm text-neutral-400">Pick a starting point, or just type a message below.</p>
      </div>
      <div className="grid w-full max-w-xl grid-cols-1 gap-3 sm:grid-cols-2">
        {SUGGESTIONS.map((s) => (
          <button
            key={s}
            className="rounded-xl border border-neutral-800 bg-neutral-900/70 px-4 py-3 text-left text-sm text-neutral-300 shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-500/40 hover:bg-neutral-800 hover:text-neutral-100"
            onClick={() => onPick(s)}
          >
            {s}
          </button>
